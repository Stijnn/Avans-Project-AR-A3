...
void ObjModel::draw()
{
	...
	for (auto& group : this->groups)
	{
		...
		{
			Texture* texture = this->materials[group->materialIndex]->texture;
			if (texture)
				texture->bind();
		}
		...
		for (auto& face : group->faces)
		{
			...
			{
				tigl::addVertex(tigl::Vertex::PTN(glm::vec3(vertex.position), glm::vec2(vertex.texcoord), glm::vec3(vertex.normal)));
			}
		}
	}
	tigl::end();
...
std::vector<tigl::Vertex> ObjModel::load(std::string fpath)
{
	...
	std::vector<tigl::Vertex> modelVertices;
	...
	else
	{
		while (1)
		{
			...
			else if (strcmp(lineHeader, "vt") == 0) //If its a UV
			{
				glm::vec2 uv;
				fscanf(file, "%f %f\n", &uv.x, &uv.y);
				temp_uvs.push_back(uv);
			}
			else if (strcmp(lineHeader, "vn") == 0) //If its a normal
			{
				glm::vec3 normal;
